...
CREATE TABLE rpsl_mntby (
    id    INTEGER PRIMARY KEY AUTOINCREMENT,
    key   TEXT    NOT NULL,                  -- Ідентифікатор об'єкту (напр. AS12345, AS-EXAMPLE)
    value TEXT    NOT NULL COLLATE NOCASE,   -- Ідентифікатор мейнтейнера (з атрибуту mnt-by:)
    mntby TEXT    NOT NULL COLLATE NOCASE,   -- Тип об'єкту-джерела: aut-num, as-set, role
    UNIQUE(mntby, key, value)
);
```

> **⚠️ Назви колонок не відповідають своєму змісту.** Це історична особливість
> реалізації, підтверджена фактичним кодом заповнення таблиці
> (`parseRpsl.java`, метод `saveRpslMntBy()`):
>
> ```java
> // SQL: INSERT OR REPLACE INTO rpsl_mntby (mntby, key, value) VALUES (?, ?, ?)
> storeInsertRpslMntBy.setString(1, this.key);       // → колонка "mntby" отримує ТИП об'єкту (aut-num/as-set/role)
> storeInsertRpslMntBy.setString(2, rpsl_mntbyKey);   // → колонка "key"   отримує ІДЕНТИФІКАТОР об'єкту
> storeInsertRpslMntBy.setString(3, mntbyValue);      // → колонка "value" отримує ІМ'Я МЕЙНТЕЙНЕРА
> ```
>
> Тобто по факту `mntby` зберігає те, що інтуїтивно очікувалося б від `key`,
> а `key` — те, що очікувалося б від `value`, який має відношення до
> об'єкта. Увесь код читання (`retrieve/retrieveMntBy.java` в
> whois-lite-local та asblockwar) написаний узгоджено з цим реальним
> розташуванням і **перевірений на живій базі**:
>
> ```sql
> -- Знайти всі aut-num/as-set, якими керує мейнтейнер YZ42-RIPE-MNT
> SELECT key FROM rpsl_mntby WHERE mntby IN ("aut-num", "as-set") AND value = "YZ42-RIPE-MNT";
> -- → AS-UKRHUB, AS-UKRHUB-v6, AS12593
> ```
>
> Якщо колись схема буде приведена у відповідність з інтуїтивними назвами —
> потрібно синхронно оновити `parseRpsl.java` (`saveRpslMntBy`) і **обидва**
> клієнти читання (whois-lite-local, asblockwar), інакше вони розійдуться.
...
rpsl (key='aut-num'/'as-set'/'role') ◄─── rpsl_mntby
    rpsl.key = rpsl_mntby.mntby AND rpsl.value = rpsl_mntby.key
    (нагадування: rpsl_mntby.mntby = тип об'єкту, rpsl_mntby.key = ідентифікатор об'єкту —
     див. попередження в розділі про rpsl_mntby)
...
```sql
SELECT rm.mntby AS object_type, rm.key AS object_id, r.block
FROM rpsl_mntby rm
JOIN rpsl r ON r.key = rm.mntby AND r.value = rm.key
WHERE UPPER(rm.value) = UPPER('EXAMPLE-MNT')
ORDER BY rm.mntby, rm.key;
```
