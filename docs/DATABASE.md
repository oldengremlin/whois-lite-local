...
CREATE TABLE rpsl_mntby (
    id    INTEGER PRIMARY KEY AUTOINCREMENT,
    key   TEXT    NOT NULL,                  -- Тип об'єкту: aut-num, as-set, role
    value TEXT    NOT NULL COLLATE NOCASE,   -- Ідентифікатор об'єкту
    mntby TEXT    NOT NULL COLLATE NOCASE,   -- Ідентифікатор мейнтейнера
