...
знайде **найточніший** inet6num, що покриває цю адресу, разом з його організацією та вкладеними route6.

Повертається саме найточніший збіг, як це робить whois. Інакше кожен IPv4-запит починався б із заглушки `inetnum: 0.0.0.0 - 255.255.255.255`, яку RIPE тримає для непризначених адрес, — і тягнув би за собою всі маршрути, що лежать усередині.

Об'єкти цих типів сильно перекриваються, тому пошук побудовано на точних збігах меж блоку: кожен збережений рядок є CIDR-блоком, тож довжини префікса перебираються від найточнішої до найширшої з зупинкою на першому влучанні. Це на два порядки швидше за `firstip <= ? AND lastip >= ? ORDER BY firstip DESC LIMIT 1`, який у непокритій зоні сканує індекс назад аж до `/0`.

Діапазон, що не зводиться до одного префікса, розкладається на точний набір блоків: `20.33.0.0 - 20.128.255.255` — це сім блоків шести різних розмірів.

## Оновлення з версій до 1.4.1

Таблиця `rpsl_net` перебудовується — стара версія видаляється автоматично, але **наповнюється вона лише під час наступного `--get-data`**. До того нові опції не знаходитимуть нічого.
